...
def get_all_keys() -> Dict[str, List[str]]:
    return {
        group_key: [type_key for type_key in types]  # noqa: C416
...
def get_type(groupKey: str, typeKey: str) -> Type:
    try:
        ...
        raise HTTPException(
            status_code=404, detail=f"No type {typeKey} for group {groupKey} exists."
        )
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"A server error occurred. Details: {e}"
...
@router.post(
    "/types/{groupKey}/{typeKey}/validate-type-schema", response_model=ValidationResult
)
